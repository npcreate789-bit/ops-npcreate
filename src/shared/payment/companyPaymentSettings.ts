...
/**
 * `Number(x)` คืน `NaN` (ไม่ใช่ null/undefined) สำหรับค่าที่แปลงไม่ได้
 * → `Number(x) ?? fallback` เป็น dead code; ต้องใช้ helper นี้ที่ตรวจ
 * `Number.isFinite` ก่อน เพื่อให้ fallback ทำงานจริงตอนเซิร์ฟเวอร์ส่ง
 * null/undefined/empty มา
 */
...
function parseSettings(data: unknown): CompanyPaymentSettings | null {
  ...
  return {
    ...
    account_name: name,
    promptpay_id: pp,
    payment_instructions_sla_hours: numericOr(r.payment_instructions_sla_hours, 48),
    payment_verification_seconds: numericOr(r.payment_verification_seconds, 90),
    auto_confirm_enabled: Boolean(r.auto_confirm_enabled),
    auto_confirm_max_amount: numericOr(r.auto_confirm_max_amount, 50000),
    amount_tolerance_baht: numericOr(r.amount_tolerance_baht, 1),
    require_reference_match: r.require_reference_match !== false,
    manual_review_min_amount: numericOr(r.manual_review_min_amount, 100000),
    ...
    bank_match_amount_tolerance_baht: numericOr(r.bank_match_amount_tolerance_baht, 1),
    bank_match_lookback_days: numericOr(r.bank_match_lookback_days, 14),
