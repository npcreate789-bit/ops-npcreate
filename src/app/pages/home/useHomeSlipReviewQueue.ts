...
export function useHomeSlipReviewQueue(roles: AppRole[], configured: boolean) {
  ...
  const [items, setItems] = useState<PaymentSlipReviewItem[]>([])
  ...
  const reload = useCallback(() => {
    ...
    setLoading(true)
    setError(null)
    void listPaymentsPendingSlipReview()
      .then(setItems)
      .catch((e) => {
        setError(e instanceof Error ? e.message : 'โหลดคิวสลิปไม่สำเร็จ')
        setItems([])
      })
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(onRefresh, SLIP_QUEUE_POLL_MS)
