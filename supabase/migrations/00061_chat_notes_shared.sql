-- โน้ตข้อความ: ทีมงานและลูกค้าในห้องที่เข้าถึงได้

CREATE OR REPLACE FUNCTION public.user_can_note_chat_message(p_message_id UUID)
RETURNS BOOLEAN
LANGUAGE sql
STABLE
SECURITY DEFINER
...
CREATE POLICY chat_message_notes_select ON public.chat_message_notes
  ...
  USING (public.user_can_note_chat_message(message_id));
...
CREATE POLICY chat_message_notes_insert ON public.chat_message_notes
  ...
  WITH CHECK (
    author_id = auth.uid()
    AND public.user_can_note_chat_message(message_id)
  );
...
CREATE POLICY chat_message_notes_update ON public.chat_message_notes
  ...
  USING (
    public.user_can_note_chat_message(message_id)
...
DECLARE
  ...
  v_trimmed TEXT;
BEGIN
  IF auth.uid() IS NULL OR NOT public.user_can_note_chat_message(p_message_id) THEN
...
BEGIN
  ...
  WHERE n.id = p_note_id
    AND public.user_can_note_chat_message(n.message_id)
    AND (n.author_id = auth.uid() OR public.is_privileged());
...
GRANT EXECUTE ON FUNCTION public.user_can_note_chat_message(UUID) TO authenticated;
