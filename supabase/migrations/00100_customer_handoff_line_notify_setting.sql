-- Phase 2C: ตั้งค่าเปิด/ปิดข้อความ "Customer Handoff" ผ่าน LINE OA
--
-- เพิ่มคอลัมน์ `line_notify_customer_handoff` (default TRUE) ใน
-- company_payment_settings — ขยาย RPC get/update ให้รองรับ
-- เพื่อให้ทีมปิดข้อความต้อนรับลูกค้าได้ ถ้าต้องการคุยมือเอง

ALTER TABLE public.company_payment_settings
  ADD COLUMN IF NOT EXISTS line_notify_customer_handoff BOOLEAN NOT NULL DEFAULT TRUE;
...
AS $$
  SELECT jsonb_build_object(
    ...
    'line_notify_customer_handoff', s.line_notify_customer_handoff,
...
CREATE OR REPLACE FUNCTION public.update_company_payment_settings(
  ...
  p_line_notify_customer_handoff BOOLEAN DEFAULT NULL
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SET
    ...
    line_notify_customer_handoff = COALESCE(
      p_line_notify_customer_handoff,
      line_notify_customer_handoff
    ),
